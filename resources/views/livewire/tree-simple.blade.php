...
<div class="w-full max-w-[100vw]" x-data="canvasTree()">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between px-4 lg:px-8 py-3.5 gap-3 border-b border-zinc-200/60 dark:border-zinc-800/80 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md sticky top-0 z-30">
        <div class="flex items-center gap-3 min-w-0">
            <div class="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-700 text-white flex items-center justify-center shadow-md shadow-indigo-600/10 flex-shrink-0">
                <span class="text-xl">📄</span>
            </div>
            <div class="min-w-0">
                <div class="flex items-center gap-2">
                    <h1 class="text-lg font-bold text-zinc-900 dark:text-zinc-100 truncate">{{ $tree->name }} <span class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 font-mono">(Simple View)</span></h1>
                    @if($tree->is_public)
                ...
                <p class="text-xs text-zinc-500 dark:text-zinc-400 truncate">{{ $tree->description ?: 'Bagan ringkas padat siap cetak' }}</p>
            </div>
        </div>
        ...
        <div class="hidden md:flex items-center gap-2.5 flex-wrap">
            ...
            <div class="flex items-center bg-zinc-100/90 dark:bg-zinc-800/90 p-1 rounded-full border border-zinc-200/60 dark:border-zinc-700/60 text-xs font-semibold">
                ...
                <a href="{{ $isPublic ? ($publicSlug ? url('/public/tree/'.$publicSlug.'/simple') : '#') : route('tree.simple', $tree) }}"
                   wire:navigate
                   class="flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all bg-white dark:bg-zinc-700 text-indigo-700 dark:text-indigo-300 shadow-sm font-bold">
                    <span>📄</span>
                    <span>Simple</span>
                </a>
            </div>
            ...
            <flux:dropdown>
                ...
                <flux:menu>
                    <flux:menu.item icon="photo" href="{{ route('tree.export', ['tree' => $tree, 'format' => 'png', 'view' => 'simple']) }}">
                        Export Gambar (PNG)
                    </flux:menu.item>
                    <flux:menu.item icon="document-arrow-down" href="{{ route('tree.export', ['tree' => $tree, 'format' => 'pdf', 'view' => 'simple']) }}">
                        Export Dokumen (PDF)
                    </flux:menu.item>
...
<script>
    Alpine.data('canvasTree', () => ({
        ...
        centerTree() {
            ...
            const treeEl = document.getElementById('simpleTree') || inner;
            ...
            if (containerW < 768) {
                optimalScale = Math.min(0.9, Math.max(0.4, scaleX));
            } else {
                optimalScale = Math.min(1.0, Math.max(0.5, optimalScale));
            }
